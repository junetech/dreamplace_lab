...
from dreamplace.ops.dct.discrete_spectral_transform import (
    get_exact_expk as precompute_expk,
)
from dreamplace.ops.electric_potential.electric_overflow import (
    ElectricDensityMapFunction as ElectricDensityMapFunction,
)
from dreamplace.ops.electric_potential.electric_overflow import (
    ElectricOverflow as ElectricOverflow,
)
...
class ElectricPotentialFunction(Function):
    ...
    def forward(
        ...
        if inv_wu2_plus_wv2 is None:
            wu = (
                torch.arange(M, dtype=density_map.dtype, device=density_map.device)
                .mul(2 * np.pi / M)
                .view([M, 1])
            )
            wv = (
                torch.arange(N, dtype=density_map.dtype, device=density_map.device)
                .mul(2 * np.pi / N)
                .view([1, N])
            )
    ...
    def backward(ctx, grad_pos):
        ...
        if grad_pos.is_cuda:
            output = -electric_potential_cuda.electric_force(
                ...
                ctx.num_filler_impacted_bins_x,
                ctx.num_filler_impacted_bins_y,
...
class ElectricPotential(ElectricOverflow):
    ...
    def __init__(
        ...
        if region_id is not None:
            ...
            if region_id < len(placedb.regions):
                self.fence_region_mask = (
                    node2fence_region_map[:num_movable_nodes] == region_id
                )
            else:
                self.fence_region_mask = node2fence_region_map[
                    :num_movable_nodes
                ] >= len(placedb.regions)

            node_size_x = torch.cat(
                [
                    node_size_x[:num_movable_nodes][self.fence_region_mask],
                    node_size_x[num_movable_nodes : num_nodes - num_filler_nodes],
                    node_size_x[
                        num_nodes
                        - num_filler_nodes
                        + placedb.filler_start_map[region_id] : num_nodes
                        - num_filler_nodes
                        + placedb.filler_start_map[region_id + 1]
                    ],
                ],
                0,
            )
            node_size_y = torch.cat(
                [
                    node_size_y[:num_movable_nodes][self.fence_region_mask],
            ...
            num_filler_nodes = (
                placedb.filler_start_map[region_id + 1]
                - placedb.filler_start_map[region_id]
            )
            if movable_macro_mask is not None:
                movable_macro_mask = movable_macro_mask[self.fence_region_mask]
            ## sorted cell is recomputed
            sorted_node_map = torch.sort(node_size_x[:num_movable_nodes])[1].to(
                torch.int32
            )
            ## make pos mask for fast forward
            self.pos_mask = torch.zeros(
                2, placedb.num_nodes, dtype=torch.bool, device=node_size_x.device
            )
            self.pos_mask[0, : placedb.num_movable_nodes].masked_fill_(
                self.fence_region_mask, 1
            )
            self.pos_mask[1, : placedb.num_movable_nodes].masked_fill_(
                self.fence_region_mask, 1
            )
            self.pos_mask[
                :,
            ...
            self.pos_mask[
                ...
                placedb.num_nodes
                - placedb.num_filler_nodes
                + placedb.filler_start_map[region_id] : placedb.num_nodes
                - placedb.num_filler_nodes
                + placedb.filler_start_map[region_id + 1],
            ] = 1
    ...
    def compute_fence_region_map(
        ...
        if macro_pos_x is not None:
            pos = torch.cat(
                [fence_region[:, 0], macro_pos_x, fence_region[:, 1], macro_pos_y], 0
            )
            num_terminals = fence_region.size(0) + macro_size_x.size(0)
            node_size_x = torch.cat(
                [fence_region[:, 2] - fence_region[:, 0], macro_size_x], 0
            )
            node_size_y = torch.cat(
                [fence_region[:, 3] - fence_region[:, 1], macro_size_y], 0
            )
        else:
    ...
    def reset(self):
        ...
        logger.info(
            "regard %d cells as movable macros in global placement"
    ...
    def forward(self, pos, mode="density"):
        ...
        if self.initial_density_map is None:
            ...
            if self.fence_regions is not None:
                if self.placedb.num_terminals > 0:
                    ...
                    self.initial_density_map = self.compute_fence_region_map(
                        ...
                        pos[
                            num_nodes
                            + self.num_movable_nodes : num_nodes
            ...
            logger.info(
                "fixed density map: average %g, max %g, bin area %g"
                % (
            ...
            # wu and wv
            wu = (
                torch.arange(M, dtype=pos.dtype, device=pos.device)
                .mul(2 * np.pi / M)
                .view([M, 1])
            )
            # scale wv because the aspect ratio of a bin may not be 1
            wv = (
                torch.arange(N, dtype=pos.dtype, device=pos.device)
                .mul(2 * np.pi / N)
                .view([1, N])
                .mul_(self.bin_size_x / self.bin_size_y)
            )
            wu2_plus_wv2 = wu.pow(2) + wv.pow(2)
            wu2_plus_wv2[0, 0] = 1.0  # avoid zero-division, it will be zeroed out
            self.inv_wu2_plus_wv2 = 1.0 / wu2_plus_wv2
            self.inv_wu2_plus_wv2[0, 0] = 0.0
            self.wu_by_wu2_plus_wv2_half = wu.mul(self.inv_wu2_plus_wv2).mul_(1.0 / 2)
        ...
        if mode == "density":
            ...
            density_cost = (
                (density_map - self.target_density * bin_area).clamp_(min=0.0).sum()
            )
